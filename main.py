...
with tab3:
    ...
    # ---------------- RAG Integration ----------------
    # Place this AFTER you compute: user_return, volatility_annual, sharpe_ratio, sortino_ratio, vs_benchmark
    # and AFTER you have df (historical fund data) and summary JSON from MC.
    ...
    # For demo, we create a tiny in-memory KB. In production ingest factsheets, AMFI notes, and curated "how to" tips.
    KB = [
        {"id": "kb_sharpe_1", "title": "Sharpe Ratio meaning",
        "text": "Sharpe ratio measures how much excess return you earn per unit of risk. A higher Sharpe means your fund is giving better risk-adjusted returns."},
        {"id": "kb_sortino_1", "title": "Sortino Ratio meaning",
        "text": "Sortino ratio focuses only on downside volatility (bad risk). It's a more forgiving version of the Sharpe ratio, ignoring positive swings."},
        {"id": "kb_benchmark_1", "title": "Benchmark comparison",
        "text": "If your fund return exceeds the benchmark, it’s outperforming; if it lags, consider rebalancing or reviewing fund expense ratio."},
        {"id": "kb_volatility_1", "title": "Volatility explanation",
        "text": "Volatility tells how much your returns fluctuate. High volatility can mean higher potential reward, but also higher risk."},
        {"id": "kb_fees_1", "title": "Expense Ratio Tip",
        "text": "High expense ratios can quietly reduce long-term returns. Prefer funds with lower fees when performance is similar."},
        {"id": "kb_rebalance_1", "title": "Rebalancing Tip",
        "text": "Rebalancing once a year helps you lock profits and control risk exposure to outperform benchmarks steadily."},
    ]
    ...
    client = OpenAI(api_key="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")  # replace with your real key

    query = "Give me mutual fund performance insights"

    # static context (retrieved knowledge)
    context = retrieve_context(query, KB)

    # dynamic context (from your live analytics)
    runtime_context = f"""
    ...
    [Task]
    Explain to a beginner what these metrics mean, and suggest one actionable improvement tip for the investor.
    """

    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "You are a financial RAG assistant specialized in simplifying mutual fund analytics for everyday investors."},
